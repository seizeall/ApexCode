...
响应速度：进度提示按 1 秒刷新；输入框支持拖拽扩大并会随内容自动增高。任务默认不限制执行轮数和工具调用次数，可随时点击“停止任务”；`CODING_AGENT_MODEL_MAX_TOKENS` 留空表示不由客户端限制输出；上下文预算默认为 1000000 字符，实际上限仍取决于 API 服务商和模型。

安全边界：真实密钥只从 `.env` 或环境变量读取；路径不能逃逸工作区；不提供删除文件工具；写文件、补丁和命令始终受安全规则约束；完全模式会拦截危险命令。会话历史保存到 `.apexcode/sessions.json`。
