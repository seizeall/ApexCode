...
`CODING_AGENT_WORKSPACE` 工作区路径（源码运行时使用；一键版固定为 `workspace`）；`CODING_AGENT_MAX_STEPS` 单任务最大轮数，默认 12；`CODING_AGENT_MAX_TOOL_CALLS` 工具调用上限，默认 40；`CODING_AGENT_COMMAND_TIMEOUT` 本地命令超时秒数，默认 30；`CODING_AGENT_MODEL_TIMEOUT` 模型请求超时秒数，默认 60；`CODING_AGENT_MODEL_RETRIES` 网络失败重试次数，默认 2。
